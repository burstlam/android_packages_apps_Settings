...
import com.android.settings.RebootReceiver;
...
public class DailyRebootScheduleService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (MemoryManagement.isDailyRebootEnabled(this)) {
            if (intent.hasExtra("reschedule")) {
                if (rescheduledCount > 4) {
                    log("too many reschedule attempts, not rebooting today");
                    // reboot tomorrow!
                    int[] rebootTime = MemoryManagement.getUserSpecifiedRebootTime(this);
